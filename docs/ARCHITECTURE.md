...
## ADR-0019 — CI-driven prod deploy: migrations + Edge Functions on merge to main
...
- **Migrations via the session pooler.** `supabase db push --db-url "$SUPABASE_DB_URL"` where
  `SUPABASE_DB_URL` is the IPv4 **session-pooler** string (same lesson as the backup job — the
  direct `db.<ref>` host is IPv6-only, unreachable from GitHub runners). `yes |` forces
...
- **Prereqs (human, one-time).** Secrets `SUPABASE_DB_URL` + `SUPABASE_ACCESS_TOKEN`, variable
  `SUPABASE_PROJECT_REF`; and **`ANTHROPIC_API_KEY` + `ALLOWED_ORIGIN=https://<prod-domain>`** set as
  Function Secrets via `supabase secrets set` (or the deployed functions 500 / CORS-block at
