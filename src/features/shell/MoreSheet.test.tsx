...
describe('MoreSheet', () => {
  ...
  it('lists the overflow actions when open', () => {
    ...
    for (const label of ['Grid view', 'Daily habits', 'Settings', 'Sign out']) {
  ...
  it('Grid view (the mobile door to the touch grid) runs onGrid and closes the sheet', () => {
    const h = handlers()
    render(<MoreSheet open {...h} />)
    fireEvent.click(screen.getByRole('button', { name: 'Grid view' }))
    expect(h.onGrid).toHaveBeenCalledTimes(1)
