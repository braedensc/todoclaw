...
- **`ListRow.tsx`** — one ranked row: quadrant-colored rank number + 4px left border, a
  ...
  recurring completion badge (`doneCount ≥ 3`, mirrors the grid card), inline-editable text
  (click → input → commit on Enter/blur, Esc cancels), a due badge (`daysUntil`) or recurring
  status, a `staging` badge, a **done control (`✓`)**, an expand/collapse toggle, and a soft
  delete (`×`).
