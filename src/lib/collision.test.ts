...
// recurringStatus (via overlapsAny) needs the user's zone to read a wall-clock nextDueOn.
...
describe('resolveCollision', () => {
  it('returns the target itself when the spot is clear', () => {
    ...
    expect(resolveCollision(0.5, 0.5, [existing], 'self', OPTS)).toEqual({ x: 0.5, y: 0.5 })
  })

  it('returns the target when the only nearby task is the excluded one', () => {
    const self = makeTask('self', 0.5, 0.5)
    expect(resolveCollision(0.5, 0.5, [self], 'self', OPTS)).toEqual({ x: 0.5, y: 0.5 })
  })

  it('spirals out to a free neighbor when the target is blocked', () => {
    const blocker = makeTask('b', 0.5, 0.5)
    const result = resolveCollision(0.5, 0.5, [blocker], 'self', OPTS)
  ...
  it('clamps the result to [0.04, 0.96] on both axes', () => {
    ...
    const result = resolveCollision(0.04, 0.04, [blocker], 'self', OPTS)
