...
- **Recurring + due date (2026-07-29):** a `due` date on a recurring task is a **one-off override** of its cadence for the current occurrence (ADR 2026-07-29-recurring-due-override). Effective status = the **nearer** of (cadence clock, `daysUntil(due)`) via `recurringTaskStatus` in `src/lib/recurring.ts` — a far-out deadline never mutes an already-overdue chore. A live deadline (`recurringDueLive`) also beats the done-today board hide, and completing the chore **clears `due`/`due_time`** (the override is consumed). **Any surface reading a recurring task's schedule must use `recurringTaskStatus`, not the cadence-only `recurringStatus`.** Three lockstep homes: `src/lib/recurring.ts`, `supabase/functions/_shared/plan-inputs.ts`, `_shared/chat-context.ts`.
