...
- **Mobile breakpoint:** `< 720px` (`useIsMobile` / Tailwind `wide`). Mobile has no grid (ADR-0028): `MobileMatrix` (quadrant overview → focus lists) is the only task surface, repositioning is the tap-based Move-to-quadrant sheet, adding is the bottom-nav ➕ sheet. **Width decides layout ONLY** (ADR 2026-07-22-capability-keyed-insets-width-keyed-shell): safe-area insets are unconditional `env()` on body, touch ergonomics key on `(max-width: 719px), (pointer: coarse)`, and standalone JS logic keys on `navigator.standalone` — never `matchMedia('(display-mode: standalone)')`, which iOS 26 home-screen apps do NOT match.
