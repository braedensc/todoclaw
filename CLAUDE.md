...
- **Recurring: due-ness is derived, `due` is the reminder anchor (2026-07-29).** A recurring chore surfaces via `recurringStatus` (`frequencyDays - daysSince(lastDoneAt)`) — `tasks.due`/`due_time` on such a task are the **reminder occurrence anchor** (`next_recurring_fire_at` phases off them and never advances them), NOT a deadline, and no board/plan reader consults them. To put a chore on a chosen day, **phase the cadence** via `lastDoneAtForOccurrenceOn` (`_shared/recurring-schedule.ts`, exposed as BabyClaw's `schedule_for_day`); this needs zero reader changes. A recurring completion sets neither `completed_at` nor the `daily_state.done` map, so **every surface that hides done tasks must also check `recurringDoneToday`** (grid `isPlaced`, MobileMatrix, ListView, `_shared/chat-context.ts`).
