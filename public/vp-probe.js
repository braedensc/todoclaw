var baseInner = null, baseVV = null;
...
(function () {
  var baseW = window.innerWidth, h = 0, applied = -1;
  function apply() {
    var vp = window.visualViewport;
    var visible = Math.round(vp ? vp.height : window.innerHeight);
    if (window.innerWidth !== baseW) { baseW = window.innerWidth; h = 0; }
...
function render() {
  ...
  var navUA = /standalone/.test(navigator.standalone ? 'standalone' : '') || window.navigator.standalone === true;
  var innerH = window.innerHeight, innerW = window.innerWidth;
  var vvH = vv ? Math.round(vv.height) : 0, vvW = vv ? Math.round(vv.width) : 0;
  ...
  var scrH = window.screen.height, scrW = window.screen.width;
  var kbInnerDelta = baseInner != null ? baseInner - innerH : 0;
  var kbVVDelta = baseVV != null ? baseVV - vvH : 0;
  var L = [];
  L.push('<span class="hi">display-mode: ' + (standalone ? 'STANDALONE ✓' : 'browser (tab)') + '</span>');
  L.push('nav.standalone   = ' + (window.navigator.standalone === true));
  L.push('');
  L.push('<span class="k">screen</span>        ' + scrW + ' x ' + scrH);
  L.push('<span class="k">innerW/H</span>      ' + innerW + ' x <span class="hi">' + innerH + '</span>');
  L.push('<span class="k">visualVP W/H</span>  ' + vvW + ' x <span class="hi">' + vvH + '</span>   offsetTop=' + vvTop);
  ...
  L.push('');
  L.push('<span class="k">safe-area T/R/B/L</span>  ' + ins('top') + ' / ' + ins('right') + ' / ' + ins('bottom') + ' / ' + ins('left'));
  L.push('');
  L.push('<span class="k">— vs screen —</span>');
  L.push('innerH - screen  = ' + (innerH - scrH) + (innerH !== scrH ? '  <span class="warn">(differs!)</span>' : ''));
  L.push('dvh - screen     = ' + (dvh - scrH) + (dvh !== scrH ? '  <span class="warn">(differs!)</span>' : ''));
  L.push('lvh - screen     = ' + (lvh - scrH) + (lvh !== scrH ? '  <span class="warn">(differs!)</span>' : ''));
  L.push('');
  L.push('<span class="k">— keyboard deltas (vs baseline) —</span>');
  L.push('baseline set:    ' + (baseInner != null ? 'yes (inner=' + baseInner + ', vv=' + baseVV + ')' : 'NO — tap the button first'));
  L.push('innerH shrink:   <span class="hi">' + kbInnerDelta + '</span>' + (kbInnerDelta > 40 ? '  <span class="warn">(inner shrinks w/ keyboard!)</span>' : ''));
...
var vv = window.visualViewport;
['resize', 'scroll'].forEach(function (e) { vv && vv.addEventListener(e, render); });
