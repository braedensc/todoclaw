...
-- Re-add the plan size cap that write_path_volume_caps (20260722100000) put on the old plaintext
-- column — dropping the column above dropped it. Same guardrail, sized up for PGP ciphertext overhead.
alter table public.daily_state
  add constraint daily_state_plan_size check (plan is null or pg_column_size(plan) <= 98304);
...
-- dispatch_inputs_for_user: DECRYPT plan on read so the evening-recap builder still sees a DayPlan.
-- Body is main's LATEST version VERBATIM (20260722190000_dispatch_waking_tasks.sql — the `waking`
-- coming-up heads-ups + size/start_date/dormancy handling) with the SINGLE change of wrapping the
-- plan read in public.dec_jsonb(). This migration runs LAST, so this decrypt-aware version wins over
-- the plaintext one that 20260722190000 installed. Kept security-definer / service_role.
...
-- chat_list_previews (20260714140000_chat_list_previews.sql): the "Your chats" preview line + reply
-- badge. Its content/meta are now ciphertext, so the returned last_content/last_meta must be DECRYPTED
-- and the hidden-turn filter must decrypt meta before reading meta->>'hidden'. Body is main's version
-- with dec_jsonb() wrapped around the two returned columns and the WHERE meta check; runs LAST so this
-- decrypt-aware version wins. Signature/return shape unchanged, so use-chat-previews is unaffected.
...
as $$
  ...
  select distinct on (m.session_id)
    ...
    m.role::text,
    public.dec_jsonb(m.content),
    public.dec_jsonb(m.meta)
  from public.chat_messages m
  join mine on mine.id = m.session_id
  where m.user_id = auth.uid()
    and not coalesce((public.dec_jsonb(m.meta) ->> 'hidden')::boolean, false)
