...
--
-- Intent: due_task_reminders() (20260712000000) selects EVERY due reminder across ALL users with no
-- bound, and dispatch-reminders processes them serially each minute. A single user with a large
-- backlog (many tasks × many offsets) could return a run so large it never finishes inside the
-- minute, degrading reminder latency for everyone. Add a per-run LIMIT so each sweep does a bounded
-- amount of work; the ORDER BY r.fire_at (oldest first, unchanged) means the most-overdue reminders
-- are always drained first, and anything above the cap is picked up by the next minute's run (it is
-- still within the 60-minute freshness window). Paired with the per-user task_reminders cap (DB-caps
-- work) so no single account can fill the batch run after run.
--
...
--
-- ----------------------------------------------------------------------------
-- Down path (manual reversal — restore the unbounded 20260712000000 signature):
--   drop function if exists public.due_task_reminders(int);
--   -- then re-run the due_task_reminders() body from 20260712000000_recurring_reminders_unify.sql
